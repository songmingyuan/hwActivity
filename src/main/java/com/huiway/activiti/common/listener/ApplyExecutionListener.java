...
@Slf4j
public class ApplyExecutionListener implements ExecutionListener{
	/**
	 * 调用接口的文档
	 */
	private String applyMethdUrl = "http://127.0.0.1:8088/hwsystem/apply/audit/person";
	private String purchaseMethdUrl = "http://127.0.0.1:8088/hwsystem/purchase/audit/person";
	@Override
	 public void notify(DelegateExecution execution) {
		log.info("进入ApplyExecutionListener执行监听器方法------------");
		...
		if(procDefId.startsWith("userApply")){
			url=applyMethdUrl;
		}else if(procDefId.startsWith("purchaseIn")){
			url=purchaseMethdUrl;
		}
